...
class RSA_KeyPair {
    public:
        ...
        // Deserialize from Datasource
        RSA_KeyPair(Datasource& ds);

        // Initialize with specific values
        RSA_KeyPair(const std::string& n, const std::string& e, const std::string& d);

        // Deserialize from JSON
        RSA_KeyPair(nlohmann::json json);

        bool operator==(const RSA_KeyPair& rhs) const;
