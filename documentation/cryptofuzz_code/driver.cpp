...
void Driver::Run(const uint8_t* data, const size_t size) const {
    ...
    static ExecutorRSA_generate_key_ex executorRSA_generate_key_ex(CF_OPERATION("RSA_generate_key_ex"), modules, options);
    ...
    try {
        ...
        switch ( operation ) {
            ...
            case CF_OPERATION("RSA_generate_key_ex"):
                executorRSA_generate_key_ex.Run(ds, payload.data(), payload.size());
            break;
        }
