...
std::optional<component::Key> OpenSSL::OpKDF_PBKDF(operation::KDF_PBKDF& op) {
...

    {
        CF_CHECK_NE(md = toEVPMD(op.digestType), nullptr);
/*
        if ( op.digestType.Get() == CF_DIGEST("SHAKE128") || op.digestType.Get() == CF_DIGEST("SHAKE256")) {
            return ret;
        }
*/
        auto passwordCopy = op.password.Get();
...
std::optional<component::RSA_KeyPair> OpenSSL::OpRSA_generate_key_ex(operation::RSA_generate_key_ex& op){
    std::optional<component::RSA_KeyPair> ret = std::nullopt;
    Datasource ds(op.modifier.GetPtr(), op.modifier.GetSize());
    ...
    RSA* rsa = nullptr;
    BIGNUM* bn_e = nullptr;
    OpenSSL_bignum::Bignum bn_e2(ds);
    char* n_str = nullptr; // n : modulus
    char* e_str = nullptr; // e : public exponent
    char* d_str = nullptr; // d : private exponent

    try{

        /* Create RSA structure and set e */
        {
            CF_CHECK_EQ(bn_e2.New(), true);
            CF_CHECK_NE(rsa = RSA_new(), nullptr); // Allocate a new RSA structure
            CF_CHECK_EQ(RSA_set0_key(rsa, nullptr, bn_e, nullptr), 1); // Set e; n and d are set by RSA_generate_key_ex

        }

        /* Generate Key */
        {
            CF_CHECK_EQ(RSA_generate_key_ex(rsa, op.bits, BN_dup(bn_e), nullptr), 1); // Generate the RSA key pair
            // After this call, RSA structure contains n,e, and d
        }

        /* Extract key components */
        {
            const BIGNUM* n = nullptr;
            const BIGNUM* e_ptr = nullptr;
            const BIGNUM* d = nullptr;
            CF_NORET(RSA_get0_key(rsa, &n, &e_ptr, &d)); // Retrieve n,e,d

            CF_CHECK_NE(n, nullptr);  // Ensure n is not null
            CF_CHECK_NE(e_ptr, nullptr); // Ensure e is not null
            CF_CHECK_NE(d, nullptr); // Ensure d is not null

            // Convert BIGNUM to decimal strings
            CF_CHECK_NE(n_str = BN_bn2dec(n), nullptr); // convert n to string
            CF_CHECK_NE(e_str = BN_bn2dec(e_ptr), nullptr); // convert e to string
            CF_CHECK_NE(d_str = BN_bn2dec(d), nullptr); // convert d to string
        }
        {
            ret = component::RSA_KeyPair(std::string(n_str), std::string(e_str), std::string(d_str));
        }
    } catch(...){ }
end:
    RSA_free(rsa);
