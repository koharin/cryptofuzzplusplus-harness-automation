...
std::string RSA_generate_key_ex::Name(void) const { return "RSA_generate_key_ex"; }
std::string RSA_generate_key_ex::ToString(void) const {
    std::stringstream ss;

    ss << "operation name: RSA_generate_key_ex" << std::endl;
    ss << "public_exponent: " << std::to_string(public_exponent).c_str() << std::endl;
    ss << "bits: " << std::to_string(bits) << std::endl;

    return ss.str();
}

nlohmann::json RSA_generate_key_ex::ToJSON(void) const {
    nlohmann::json j;
    j["operation"] = "RSA_generate_key_ex";
    j["public_exponent"] = std::to_string(public_exponent).c_str();
    j["bits"] = bits;
    j["modifier"] = modifier.ToJSON();
    return j;
}
