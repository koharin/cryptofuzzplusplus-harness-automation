...
RSA_KeyPair::RSA_KeyPair(const std::string& n, const std::string& e, const std::string& d) :
    n(n),
    e(e),
    d(d)
{ }

bool RSA_KeyPair::operator==(const RSA_KeyPair& rhs) const {
    return
        (n == rhs.n) &&
        (e == rhs.e) &&
        (d == rhs.d);
...
RSA_KeyPair::RSA_KeyPair(nlohmann::json json) {
    n = json["n"].get<std::string>();
    e = json["e"].get<std::string>();
    d = json["d"].get<std::string>();
}

/*
void RSA_KeyPair::Serialize(Datasource& ds) const {
    ds.Put<std::string>(n);
    ds.Put<std::string>(e);
    ds.Put<std::string>(d);
}
*/
nlohmann::json RSA_KeyPair::ToJSON() const {
    nlohmann::json j;
    j["n"] = n;
    j["e"] = e;
    j["d"] = d;
