...
def openssl_module_h_preprocess(origin_path, output_file):
    """preprocess openssl_module.h"""
    pattern="> Op"
    module_file=os.path.join(origin_path, "openssl_module.h")
        ...
        if pattern in line:
            ...
            line_format = {
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": f"Generate {algorithm_name} code for openssl_module.h"
                    },
                    {
                        "role": "assistant",
                        "content": line
            ...
            with open(output_file, 'a', encoding='utf-8') as file:
                ...
                file.write(json_str + '\n')
            continue    
    print("[+] End of preprocess openssl_module.h and save to dataset")
...
def start():
    origin_path=os.path.join(os.getcwd(), "data", "cryptofuzz")
    output_path=os.path.join(os.getcwd(), "data", "preprocess")
    os.makedirs(output_path, exist_ok=True)  
    output_file=os.path.join(output_path, "output.jsonl")

    module_cpp_preprocess(origin_path, output_file)
    openssl_module_h_preprocess(origin_path, output_file)
